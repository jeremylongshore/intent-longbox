...
**Sixty-one tables, sixty-one owners, no table owned twice and none unowned** (v1.3.0: **thirty-one created, thirty not yet created** — v1.2.0 read fifty-six tables, seventeen created; E04-D01's `016` and `017` created fourteen of the mapped rows and added five new ones, all `catalog`'s).

> **What this map still does not cover, said plainly rather than left as an absence.** It maps the tables the *domain* records name. It does **not** yet carry the platform tables landed after this record was drafted — `listing_status_observation` (`005`), `request_idempotency` (`009`), `scan_session_transition` (`010`), `outbox` and `outbox_attempt` (`011`) — nor the migration runner's own `schema_migrations`. Each has an owner in its own ratified record (040, 042, 043) and each is declared in `src/db/appendOnlyTables.ts`, so none is unowned in the system; what is missing is the row that makes the ownership checkable **here**, which is exactly the gap §9's clause exists to close. That is a separate amend-by-a-row and **this entry does not make it** — E04-D02's scope is the catalog module. Recorded so the next reader finds a named gap instead of assuming the map is complete. Migration 002 adds no table (it widens the `shop_credentials.kind` CHECK). `labor_shift` appears in two CHECK enums in 003 (`:187`, `:215`) but **the table does not exist**; v1.1.1 said it belongs to reporting when E11/E12 create it and that this record should be amended by a row rather than assumed — **that row is applied at v1.2.0** (034 §7), and `labor_shift` and `labor_shift_confirmation` are now named in §2.8's owned-table list.
