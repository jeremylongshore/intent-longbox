...
`vertical: "sports-card"`; definition `{ sport, player, set, year, manufacturer }`; edition `{ number, variant, parallel, language, grader, cert_number }`; signature `set|number|variant|language|grader`; capture `front + back` required, `slab label` conditional on `grader != null`; condition `gradeLabels: ["POOR","GOOD","VG","EX","NM","MINT","GEM"]` with `graderLabelMap` translating a PSA or BGS label into a Longbox **range** — never carrying the number through (022 P1, 019 T7); crosswalk namespaces `upc`, `pricecharting`, `ebay_epid`, `psa_cert`.
