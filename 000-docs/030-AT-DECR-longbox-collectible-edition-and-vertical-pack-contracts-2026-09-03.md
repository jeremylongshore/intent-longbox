...
| **1.3.1** | **2026-09-04** | **Patch (E04-B04). A statement of fact is repaired and no decision changed.** §5.4's card sketch offers `MINT` and `GEM` bands; **021 gained B21 — *"'Mint' as a Longbox band"* — after that sketch was written**, on the ground it quotes from Mile High Comics at 037 §2.0.5 (*"'Mint' is a standard that is too difficult to define"*). Pack certification now refuses either band **in every vertical** (`certifyConditionSchema`, `src/catalog/packManifest.ts`), and the two shipped card manifests carry `["POOR","GOOD","VG","EX","NM"]`. The sketch's text is LEFT AS WRITTEN with a note beneath it, rather than edited: a ratified sketch that quietly changed would hide the fact that a blocklist row overtook it, and 049 §8 already assigns the authoritative card condition vocabulary to **E08** (E08-B02 per 014 §8 / 015, `longbox-e5b.8.2`), not to this record. Nothing in §2, §3, §4, §5.1–§5.3, §6, §7 or §8 moved; no invariant changed; no comic behaviour changed. Recorded in 051 §8, the 006 row of 2026-09-04, and the 000-INDEX / 016 rows for 030. | E04-B04 → acting head |
| **1.3.0** | **2026-09-04** | **Amend-by-a-row at E04-B03's ratification (049 §5.2). §5.4 card signature: `parallel` ENTERS the key — `set\|number\|variant\|language` becomes `set\|number\|variant\|parallel\|language`.** §5.4 listed `parallel` as a card edition attribute and left it out of the sketched signature, so a Silver Prizm #12 and a base #12 produced the SAME signature and deduped as candidates for each other. A parallel is a different PRINTING of the same card, which is exactly what an `edition` is (§2.1), and two parallels trade at different prices — a key that cannot separate them files an unbounded human queue on the ordinary case. A **minor** bump because the content of a decision changed — the card sketch's key — even though no rule in §2, §3, §4, §6 or §7 moved, no ratified invariant changed, and **no comic behaviour changed at all**. §3.3's card bullet is corrected in the same edit, so the record does not carry two signatures again (the defect v1.2.1 repaired). **The same edit also adds `printRun` to §5.4's card edition attribute list**, and it is recorded here rather than slipped in: 049 §5.5 rules that the RUN a parallel was printed to is an EDITION fact every copy shares, while the serial number stamped on one card is a COPY fact — the same line 047 A6 draws for a grader, drawn again for the card domain's own shape. `printRun` is an attribute and is deliberately NOT in the signature. **Why here rather than in 049:** E04-B03 owns the card identity schema, but §5.4 is a ratified sketch, and leaving it contradicting the shipped pack would make the contradiction something a future pack author inherits — the reasoning v1.2.0 used, and E19-B06 still gates any second vertical, so the sketch is amended while it is still a sketch. 006 row dated 2026-09-04. | 049 §5.2 → acting head |
| **1.2.1** | **2026-09-04** | Patch (E04-B01 gate audit): §3.3 card signature aligned with the v1.2.0 §5.4 amendment — `grader` is a copy fact, 047 A6. The v1.2.0 row amended §5.4's sketched signature but left §3.3's bullet list still reading `set + number + variant + language + grader`, so one ratified record carried both signatures. The bullet now reads `set + number + variant + language`, with a pointer to §5.4. **No decision changed** — the decision was taken at v1.2.0; this repairs the statement of it in a second place. | E04-B01 gate audit (`longbox-gate-auditor`) |
...
- **Cards:** `set` + `number` + `variant` + `parallel` + `language` (per the E02-B04 brief and 014 §3.4's "identity schema and exact keys"; `grader` left this list at v1.2.1 — it is a copy fact, per 047 A6 — and `parallel` JOINED it at v1.3.0, because a parallel is a different printing and not a decoration: 049 §5.2, and §5.4 below).
...
`vertical: "sports-card"`; definition `{ sport, player, set, year, manufacturer }`; edition `{ number, variant, parallel, language, printRun }`; signature `set|number|variant|parallel|language` (v1.3.0 — `parallel` entered the key at E04-B03; 049 §5.2); capture `front + back` required, `slab label` conditional on the **copy** carrying a certification; condition `gradeLabels: ["POOR","GOOD","VG","EX","NM","MINT","GEM"]` — **the last two of which no pack may now ship; see the note below** — with `graderLabelMap` translating a PSA or BGS label into a Longbox **range** — never carrying the number through (022 P1, 019 T7); crosswalk namespaces `upc`, `pricecharting`, `ebay_epid`.

**⚠ `MINT` AND `GEM` ARE REFUSED BY PACK CERTIFICATION, AND THE SKETCH ABOVE IS LEFT AS WRITTEN (v1.3.1, a patch).** 021 gained **B21** — *"'Mint' as a Longbox band"* — on 2026-09-04, after this sketch, on the ground quoted there from Mile High Comics at 037 §2.0.5: *"'Mint' is a standard that is too difficult to define. All comics have some flaws."* `certifyConditionSchema` (E04-B04, `src/catalog/packManifest.ts`) therefore refuses any grade label folding to MINT or GEM **in every vertical**, and the two shipped card manifests carry the five bands that survive: `["POOR","GOOD","VG","EX","NM"]`. **No decision in this record changed** — a sketch is not a ratified vocabulary, the authoritative card condition schema is **E08's** (049 §8; E08-B02 per 014 §8 / 015, `longbox-e5b.8.2`), and this row records where the sketch and the shipped floor now differ rather than editing the sketch to hide it. See 051 §8.
