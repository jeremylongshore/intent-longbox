...
`vertical: "sports-card"`; definition `{ sport, player, set, year, manufacturer }`; edition `{ number, variant, parallel, language, printRun }`; signature `set|number|variant|parallel|language` (v1.3.0 — `parallel` entered the key at E04-B03; 049 §5.2); capture `front + back` required, `slab label` conditional on the **copy** carrying a certification; condition `gradeLabels: ["POOR","GOOD","VG","EX","NM","MINT","GEM"]` with `graderLabelMap` translating a PSA or BGS label into a Longbox **range** — never carrying the number through (022 P1, 019 T7); crosswalk namespaces `upc`, `pricecharting`, `ebay_epid`.
