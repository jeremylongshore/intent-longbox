...
**Version:** 1.3.1 — patch (amend-by-a-row from E03-D17, 2026-09-06): **§3.3's accessor contract is BUILT** — `src/identity/` is the module, `identity_access` is the audit row, and the change-log row below names the clauses that are discharged and the two that are not (the break-glass gate is E11-B09's, the heartbeat is E13-B04-D1's), so 019 T35(b) moves **OPEN → INSTRUMENTED and not to closed**. §3.3's own text is unchanged. v1.3.0 — minor (amend-by-a-row from E03-D09, 2026-09-04): §4.3's A4 keeps its GUARANTEE and changes its MECHANISM — `shop.organization_id` is enforced by a validated `CHECK (organization_id IS NOT NULL)` rather than by `SET NOT NULL`, because 000-docs/044 §2 (which did not exist when A4 was written) makes `SET NOT NULL` a contracting statement. v1.2.0 was the 048 §7.4 amend-by-a-row making `device_credential.key_ref` into `token_hash`. §11 stays signed. See the change log.
...
| --- | --- | --- | --- |
| **1.3.1** | **2026-09-06** | **Patch — §3.3's accessor contract is BUILT, and this row says exactly which of its clauses are discharged and which are not. A PATCH and not a minor, for CONSISTENCY: 048 and 054 took patches for this same class of change — a statement of fact about another bead's delivery, with no decision in the host record moving — and §3.3's own text is unchanged.** E03-D17 (`longbox-e5b.3.27`, record **000-docs/060**, migration `035`) lands the module §3.3 specifies: **`src/identity/`**, one public barrel, enforced by `.dependency-cruiser.cjs`'s `identity-public-surface-only` + `identity-reaches-only-platform` and by `pnpm arch`'s `identity-is-the-only-person-join` over `src/` **and** `scripts/`. **DISCHARGED:** *"exactly one module … may `SELECT` `operator_id`, `created_by` or `confirmed_by`"* (the rule forbids all three in any projection outside the module, and additionally forbids reading `app_user` at all beyond one declared exemption — the sign-in's id-only lookup, argued in 060 §5.3); *"through one named, audited accessor function"* (**three accessors over four purposes** — the data-model lens's Hickey 4 collapsed the two that issued the identical statement, and one accessor may carry more than one purpose because the declaration polices the triple); *"writes an access-audit row before returning"* (`identity_access`, `migrations/035`); *"No other module, route, view, export or report may name those columns in a projection"*. **I9's five named call sites are moved or gone**: `readUser` and the unreferenced `readPerson` were two copies of one statement and are now one accessor, `shopRoster` moved, `upsertPerson` stopped returning a display name, and `pnpm enroll-authenticator`'s email projection went through the accessor with it. **NOT DISCHARGED, and named rather than dropped:** *"requires the `support_break_glass` role"* — the four gated reads are authentication affordances on the caller's own principal and the role holds NOTHING (054 §3), so the the role holds NOTHING (054 §3), so requiring it would make the counter flow unreachable rather than gated — and what bounds the two reads that DO resolve third parties (the roster and the enrollment CLI) is a live DEVICE session, a two-column projection and an activity-independent sort (048 R8), never a role. The role belongs to a surface that renders a NAMED person's RECORD to somebody who is not them, and that surface is **E11-B09**'s and does not exist yet. *"covered by the T34 heartbeat"* — **E13-B04-D1 (`longbox-e5b.13.4.1`)**'s. **019 T35(b) therefore moves OPEN → INSTRUMENTED and not to closed** (019 v1.6.0 §9.5 A10). **Nothing in §2 or §4 is disturbed and no decision in this record is re-argued**; §3.3's own text is unchanged, which is what makes this a change-log row rather than an amendment block. | amend-by-a-row from E03-D17 (000-docs/060) |
...
| **1.3.0** | **2026-09-04** | **Minor — §4.3's A4 keeps its guarantee and changes its mechanism, by an amend-by-a-row from the bead that delivered it.** A4 instructs that `shop.organization_id` be "added nullable, populated, and set `NOT NULL` in the same transaction". `migrations/019` (E03-D09) does the first two and then adds a **validated `CHECK (organization_id IS NOT NULL)`** in place of the third, because **000-docs/044 §2 — a standard ratified after this record — makes `ALTER COLUMN … SET NOT NULL` a CONTRACTING statement**, refused by the migration lint without a `-- contract:` header naming an expand migration it retires and a 006 row authorising it. `019` retires nothing, so that header would be false. **The guarantee A4 argued for is delivered in full**: the population and the constraint are in one transaction, no committed state has a shop with no legal party, and a writer that omits the column is refused. **A minor and not a patch** because the DDL a record instructs is content, not a description of the tree (006 `:6`, the test v1.2.0 applied to a column's meaning) — a reader who quoted A4's `SET NOT NULL` must re-read §4.3. **Recorded here rather than only in the migration's header**, which is where the deviation was first written: a reader of §4.3 alone, months from now, would otherwise find a CHECK where the record says `SET NOT NULL` and have no way to tell a deliberate substitution from a mistake. Raised by the advisory PR review on PR #71 as its first finding, and the finding was right. §11 stays signed; no other section moved. | E03-D09 → under §7's amend-by-a-row |
...
|---|---|
| ~~`shop.organization_id`~~ **— not nullable (A4); enforced by a CHECK, not by `SET NOT NULL` (v1.3.0)** | **The guarantee is unchanged and the DDL is not.** A4 instructs "added nullable, populated, and set `NOT NULL` in the same transaction", and `migrations/019` delivers the first two and then adds a validated `CHECK (organization_id IS NOT NULL)` instead of the third. The reason is a standard that did not exist when A4 was written: **000-docs/044 §2 makes `ALTER COLUMN … SET NOT NULL` one of four CONTRACTING shapes**, refused without a `-- contract:` header naming the expand migration it retires and the 000-docs/006 row authorising it — and `019` retires nothing, so the header would be a claim the file does not support. A validated CHECK is the same guarantee reached by an expand-only statement: no committed state has a shop with no legal party, and a writer that omits the column is refused. **What is lost is one line of catalogue metadata** — `information_schema` reports the column nullable — and converting the CHECK into a true `NOT NULL` is a one-line contract step for whoever next needs that flag, buying no guarantee the schema lacks today. v1.0.0 left it nullable and deferred the `NOT NULL` to a later contract step, on a lock-duration argument. The cannon was right that this is the wrong trade at this size: the table has **one row**, so the lock is measured in microseconds, and the deferral buys nothing but a second migration and a window in which a `shop` can exist with no legal party to bind a consent or charter row to. The column is added nullable, populated, and set `NOT NULL` in the same transaction (§4.1 steps 2–3). **This is the single data statement in the migration**, and it is not the kind of backfill §4.4 forbids: it asserts a fact about a *business* that is externally checkable — the shop has an owner entity, and it is the entity on the pilot agreement — not a fact about a *person's actions* that was never recorded |
