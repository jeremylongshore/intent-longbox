...
**Version:** 1.1.1 — patch (gate audit): 019 §2 is the segments section (four pointers were §3.2); the 031 staffing figures re-grounded (30% no employees is a minority); shop_id added to batch_close, task_event and device_credential_revocation; I4 no longer claims an existing scan guards key_ref. No decision changed.
...
| `key_ref text NOT NULL` | **the NAME of a reference, never a key or a public key blob** — the `shop_credentials:34-35` rule, applied verbatim (locked decision 2) |
...
| **I4** | **`device_credential.key_ref` never holds a secret.** The test asserts it directly against `001:34-35` and locked decision 2; no existing scan guards this today.: no value matching a key-shaped string may be inserted; the column holds an env-var NAME (`001:34-35`) | `tests/device-credential-keyref.test.ts` |
...
- **Authentication and authorization mechanics — E03-B02.** Password/passkey storage, MFA for privileged roles, session and cookie handling, CSRF, invitations, brute-force controls, and how a `device_credential.key_ref` is actually verified. `app_user` and `device` are the *subjects*; the mechanism is E03-B02's table and E03-B02's decision.
