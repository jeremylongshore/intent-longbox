...
| 1.3.0 | 2026-09-04 | Amend-by-a-row (decision changed): F3 keys the one-tap refusal on `llm_rerank.band !== 'high'` rather than on the contradiction flag alone — E06-D01 (`longbox-e5b.6.11`, PR #64) made the band a server-side derivation of five ceilings after 046 R-3 showed a null-evidence, max-confidence response took the high band with no contradiction. I7 gains the case `contradiction=false, band='low'` → refused. Ratified by the acting head from the `longbox-invariant-reviewer` report on PR #64. | acting head |
...
- **F3 — A result the server could not corroborate removes the one-tap path.** When the driving `llm_rerank.band` is not `high`, a `human_confirmation` with `source='one_tap'` is refused (locked decision 7; 033 §5.4; 022 P1; 046 §5 A8 / R-3). Since E06-D01 the band is the server's own derivation — the minimum of five ceilings (evidence completeness, the contradiction verdict, barcode agreement, candidate uniqueness, the model's number, which can only lower it) — so the band, not the contradiction flag alone, is the corroboration signal; a contradiction is one of the ceilings that produce a non-high band. The band is downgraded and the session renders as a forced pick. This is a **schema-level** rule, not a UI convention: the API rejects it, so a client that skips the downgrade cannot produce the row. *(v1.3.0: amended by a row from E06-D01; the v1.2.x text keyed the refusal on `contradiction` alone, which four of the five ceilings could bypass.)*
...
**Three contract properties this fixes beyond the state machine.** Every mutating call carries actor, tenant, correlation id, idempotency key and schema version (E02-B08's own acceptance; **029 §11 at `029:704` and §12.3 at `029:739`, plus 034 §3.1's `RequestContext` at `034:391-399`, which names `correlation_id` and attributes it to E02-B08** — v1.2.1: this sentence cited *029 §10*, which is 029's *Consequences* section and assigns no envelope; 000-docs/042 §1 E23 found it). 005 §API's `?status=in_progress` is retired as a name — the parameter is `state`, its vocabulary is §3.2's, and 005 gets a Version bump when E02-B08 lands rather than now. And **the `SELECT *` leak is closed by name (A8)**: E5 records that the dead column is *returned in a response body today* even though nothing branches on it, which means a client could already be reading it — so removing the column without first fixing the projection would be a breaking change discovered by a consumer rather than by us.
