...
CREATE INDEX ON listing_status_observation (listing_link_id, created_at DESC);
```

Append-only, **`commerce`-owned** (029 §2.7 already claims "webhook/poll reconciliation, and the observed listing lifecycle"). Its derived view `listing_link_current_status` is `DISTINCT ON (listing_link_id) … ORDER BY created_at DESC` — the `003:108-124` shape.
...
ALTER TABLE human_confirmation
  ADD COLUMN IF NOT EXISTS observed_current_id uuid REFERENCES human_confirmation(id);
```
...
|---|---|---|
| `POST …/confirm` | appends `human_confirmation`, then `setSessionStatus(…, "confirmed")` (`routes:208-214`) | appends `human_confirmation` **only**; accepts `observed_current_id` (§5.1) and `Idempotency-Key`; **rejects `source='one_tap'` on a contradiction (F3)** |
...
2. **`listing_status_observation`** (§4.4) — **after** 036's `listing_link`, which it FKs. Index `(listing_link_id, created_at DESC)`.
3. **`request_idempotency`** (§5.2) with `UNIQUE (shop_id, idempotency_key)` — `platform`-owned.
4. `ALTER TABLE human_confirmation ADD COLUMN IF NOT EXISTS observed_current_id uuid REFERENCES human_confirmation(id)` (§5.1).
