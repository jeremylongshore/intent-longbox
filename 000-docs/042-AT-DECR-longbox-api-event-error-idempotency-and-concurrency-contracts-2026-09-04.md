...
| 1.4.4 | 2026-09-05 | **Patch (statement of fact) — §6's causal reference is now PERSISTED, and §3.3's declared projection grows by two fields on two event DTOs (E02-D11 `longbox-e5b.2.21`, migration `030`). No decision changed, no rule moved, §14 stays signed.** §6.1 fixed the WIRE shape *"and nothing else"*, and 041 §3.5 owned the columns; until this bead the check ran at write time and left nothing behind, which `src/services/worldView.ts` said in its own header. `human_confirmation` and `condition_assessment` now store the reference their write was checked against, and `eventDtos` for those two publish `against_table` / `against_id` — **a reference to another record of the same session, the same kind of field as `supersedes_id` beside it**, carrying no confidence, provider, model, cost, operator identifier or `storage_url` (§3.5, I9; 019 T35). It is an ADDITIVE `v1` change under §2.3 — a client that ignores the fields is unaffected — and `contracts/openapi.v1.json` is regenerated in the same commit, so I15 gates it as a reviewable diff. **§6.5 is unchanged and now has a visible consequence**: `against` stays optional, and a row written without one stores NULL for both columns, which is the counted fallback recorded honestly rather than guessed at. | E02-D11 build → acting head |
| **1.4.3** | **2026-09-04** | **Minor — §5.1 gains a SECOND exemption class, from an amend-by-a-row from E03-B06 (000-docs/053 §8.3). Nothing about class one moves, and the header rule is unchanged for every route that can send one.** Class one is about a route whose EFFECT the storage cannot hold; class two is about a route whose CALLER cannot participate. **An INBOUND PROVIDER CALLBACK** — a request from a third-party system that sends no cookie, no `Sec-Fetch-Site` and no `Idempotency-Key`, and can be made to send none of them — requires no header and takes no `request_idempotency` row, **because both mechanisms are REPLACED by strictly stronger ones**: a signature over the request's own bytes verified before any read or write (a cross-site form cannot produce one, which is stronger than "this request claims to be same-origin"), and a UNIQUE constraint on THE ACT that the member NAMES. The naming rule is v1.4.2's, unchanged and for its reason — every table here has a surrogate primary key, so "a UNIQUE constraint" unqualified is satisfied by `id uuid PRIMARY KEY`. **TWO members**: `GET …/connectors/shopify/callback` naming `connector_install_state_use (state_id)`, and `POST …/connectors/shopify/webhooks` naming `connector_webhook_receipt (connector, webhook_id)`. **The boundary is three conjuncts and the class ASSERTS its own membership** (which class one deliberately does not): a contract test reads each named index out of `migrations/` and fails when it is absent, and a second fails when a `provider-callback` row carries a principal. **A minor and not a patch**: a class is a decision, not a repaired statement of fact. | E03-B06 → acting head |
...
| **1.4.2** | **2026-09-04** | **Patch — §5.1's clause (b) becomes two DISJUNCTS, because as drafted at v1.4.1 it expelled three of the class's own four members, and the enumeration under it said "two" and listed one.** v1.4.1 tightened (b) to *"a credential the caller receives once — a `Set-Cookie` or a shown-once secret — **and** where the database already makes THE ACT exactly-once by a UNIQUE constraint that the member names"*. The conjunction is the error: **`POST /api/v1/device-sessions`, `POST /api/v1/operator-sessions` and `POST /api/v1/operator-sessions/end` name no such constraint and cannot** — nothing in `app_session` makes a repeated sign-in a failed INSERT, and nothing should, because §5.1's own reasoning is that a duplicate here is one more row on a chain the phone stops using. Read literally, v1.4.1 threw out the three routes the class was created for and left the one route E03-D07 added. **(b) now reads as either disjunct: (b1) the entire externally visible effect is a `Set-Cookie` and nothing else; or (b2) a shown-once secret PLUS a UNIQUE on the act, named by table and columns.** The three session routes sit under (b1) and owe no constraint; **only `POST /api/v1/device-enrollments` sits under (b2)** — its effect outlives the response (a `device`, a `device_credential` and a use row) — and it names **`device_enrollment_code_use (code_id)`**. The naming requirement is unchanged and is why (b2) exists at all: every table here has a surrogate primary key, which is a UNIQUE, so "a UNIQUE constraint" unqualified is satisfied by `id uuid PRIMARY KEY` on any route in the system. **The enumeration is also corrected from "the two members" to the four**, which is what v1.3.1 already made it (three) plus E03-D07's one. **A patch and not a minor**: no route enters or leaves the class — all four were always in it — and what is repaired is a boundary that did not describe its own membership. Raised by the re-review of `ef38fc7`. | E03-D07 re-review → acting head |
| **1.4.1** | **2026-09-04** | **Patch — §5.1's exemption clause (b) is TIGHTENED, no decision changed and no membership moved.** v1.4.0 admitted a route to the class when "the database already makes the act exactly-once by a UNIQUE constraint", and that phrase is not a test: **every table in this schema has a surrogate primary key, which is a UNIQUE**, so the clause as written is satisfied by `id uuid PRIMARY KEY` on any route in the system — the opposite of the narrowing it was added to be. The clause now requires each member to **NAME the table and columns** of the constraint that makes THE ACT exactly-once, and the one member names it: `POST /api/v1/device-enrollments` → **`device_enrollment_code_use (code_id)`**. The distinction is the one that matters — a constraint on the act is one whose violation makes a second execution a failed INSERT rather than a second effect, which `device (id)` is not. **A patch and not a minor**: the class's membership is unchanged (the same one route qualifies, for the same reason), and what is repaired is a criterion that could not do the work it claimed. Raised by the invariant review of E03-D07. | E03-D07 invariant review → acting head |
| **1.4.0** | **2026-09-04** | **Minor — §5.1's exemption class keeps its shape and its boundary widens by ONE clause, from an amend-by-a-row from E03-D07. The two routes E03-D07 adds fall on OPPOSITE sides of it, and v1.3.0's forward-looking sentence about one of them is corrected.** v1.3.0 predicted the class's next members "will be E03-D06's password sign-in and **E03-D07's enrollment redemption**". It also wrote the boundary, and the boundary decides the question against half of its own prediction. **`POST /api/v1/invitations/redemptions` is NOT exempt**: it writes a `membership` — the record of who may act in a shop, which is the substrate 019 T35(c) reconciles against — and it sets an `operator_pin`; both outlive the response, neither is a cookie, and a duplicate of either is a second grant and a second credential rather than one spare row on a chain the phone stops using. It takes the header AND the `request_idempotency` row, and the ORDER matters: `replayIfSettled` runs before the code is looked at, so a retry after a lost response replays instead of finding the code its own first call spent. **`POST /api/v1/device-enrollments` IS exempt, and clause (b) widens to admit it** — from *"whose entire externally visible effect is a cookie"* to **"whose entire externally visible effect is a credential the caller receives once — a `Set-Cookie` or a shown-once secret — AND where the database already makes the act exactly-once by a UNIQUE constraint"**. Its effect is a cookie plus three durable rows, so it does not qualify as written; it is admitted on the strength of §5.1's own argument for the class, which is that a stored replay would return a byte-identical `201` **with no cookie**, leaving a phone that believes it is enrolled and holds nothing. **The added conjunct is what stops the widening spreading**: `UNIQUE (code_id)` on `device_enrollment_code_use` is a stronger exactly-once guarantee than the idempotency key it replaces, taken by the database, on the act itself — and no route that writes a book, a batch or a catalog row has such a constraint, so none can enter the class through this door. **The cost is stated rather than discovered**: a retry after a lost response finds the code spent and is refused, so the shop issues another — a minute of an owner's time on a flow performed once per phone. **A minor and not a patch**: the class's membership test changed, which is content a reader must have. §14 stays signed; §5.2, §5.3 and §5.4 are untouched. | E03-D07 → acting head, under §14's amend-by-a-row |
| **1.3.1** | **2026-09-04** | **Patch — a statement of fact repaired, no decision changed.** v1.3.0's §5.1 exemption block enumerated FOUR routes and one of them, `GET /api/v1/operators`, is a read: §5.1 governs mutating routes only, so a GET was never inside the rule and there is nothing for it to be exempted from. The enumeration now names the three POSTs. **The exemption CLASS is untouched** — "an authentication act, whose only effect is a `Set-Cookie`" — and so is every sentence of its reasoning; what was wrong was the list, which is exactly what 006 `:6` calls a patch. Raised by the E03-D09 re-verification. | E03-D09 re-verification → acting head |
| **1.3.0** | **2026-09-04** | **Minor — one stated exemption class added to §5.1 by an amend-by-a-row from E03-D09; the header rule is unchanged and nothing else moved.** §5.1 requires an `Idempotency-Key` on every mutating route and §5.3 serialises the act through a `request_idempotency` row whose stored value is the RESPONSE BODY. E03-D09's four authentication routes keep the header — 048 §5.2's second CSRF mechanism is exactly that a cross-site form cannot set one, and the routes that mint credentials are the last place to drop it — and **do not take the row**, because an authentication act's effect is its `Set-Cookie`, which the table does not hold: a replayed `201` would be a byte-identical success with no cookie, leaving a phone that believes it is signed in and holds nothing. **A CLASS, not a route list** ("an authentication act, whose only effect is a `Set-Cookie`"), because E03-D06's password sign-in and E03-D07's enrollment redemption have the same shape — with the boundary written so it cannot spread to a route that writes a witness row. **A minor and not a patch**: §5.1's *"not 'should' and not 'on the paths that matter'"* now has a named exception, which is content a reader must have. Filed here rather than only in the service's header comment, for the reason 034 v1.3.0 was: a deviation recorded only where it deviates is one the next reader of the rule finds as a contradiction. §14 stays signed. | E03-D09 → acting head, under §14's amend-by-a-row |
| **1.2.0** | **2026-09-04** | **Minor — one column shape changed, by execution rather than by argument.** §5.2's table gave `response_status` and `response_body` **`NOT NULL`**, and §5.3 step 1 requires the handler to *"INSERT the `request_idempotency` row first, inside `withTransaction`, then do the work, then update the row's response fields"*. **Those two sentences cannot both be satisfied**: at INSERT time the response does not exist, so `NOT NULL` makes the ratified sequence unimplementable. Found by E02-B10 while building the table (`migrations/009`). **The response columns become NULLABLE, and a NULL response MEANS in flight.** §14 stays signed and §5.3's construction is untouched, because this does not reintroduce the status column §5.3 removed: §5.3's own argument is that no reader ever consults an in-flight state — a concurrent request with the same key **blocks** on `UNIQUE (shop_id, idempotency_key)` until the first transaction resolves, and then sees either a committed row (complete by construction) or no row (rolled back). The in-flight row is **unobservable from outside the writing transaction**, so the nullability is a fact about the write sequence, not a state machine. A minor and not a patch because a column's nullability is a decision, not a statement of fact. 006 decision log carries the row. | E02-B10 build (`longbox-e5b.2.10`) → acting head |
...
**ONE EXEMPTION, AND IT IS A CLASS RATHER THAN A ROUTE (v1.3.0, from E03-D09).**

> **An AUTHENTICATION ACT — one whose only effect is a `Set-Cookie` — requires the header and does NOT take a `request_idempotency` row.**

The class is exactly THREE routes today — 048 §6.2's `POST …/device-sessions`, `POST …/operator-sessions` and `POST …/operator-sessions/end` — and the rule is stated as a class because the next one will be E03-D06's password sign-in and E03-D07's enrollment redemption, which have the same shape. *(v1.3.1 removes `GET …/operators` from the enumeration: §5.1 has only ever governed MUTATING routes, so a GET was never inside the rule and cannot be exempted from it. The class is unchanged; the list was wrong.)*

**The header requirement is UNCHANGED and is load-bearing on these routes in particular**: 048 §5.2's second CSRF mechanism is that *"a cross-site HTML form cannot set a custom header"*, so the routes that mint credentials are the last place to drop it. What is skipped is §5.3's STORAGE, and the reason is that §5.3 stores a **response body**. An authentication act's effect is not its body — it is the cookie, which `request_idempotency` does not hold and cannot: replaying a stored `201` would return a byte-identical success **with no `Set-Cookie`**, leaving a phone that believes it is signed in and holds nothing. That failure is worse than the duplicate the storage exists to prevent, and the duplicate here is cheap: a second operator session revokes the first as `operator_switch` (048 §3.5), and a second device session is one more row on a chain the phone stops using.

**The boundary of the exemption, so it cannot spread.** It covers a route that **(a)** writes no witness row about a book, a batch or a shop's catalog, **and (b)** whose entire externally visible effect is a credential the caller receives once — which is satisfied by EITHER of two disjuncts, never by a conjunction of them: **(b1) a `Set-Cookie` and nothing else**, or **(b2) a shown-once secret PLUS a UNIQUE on THE ACT that the member names by table and columns.** `POST …/scan-sessions` is in neither and never can be.

**The FOUR members, and which of them owes a constraint** (v1.4.2). The class is `POST /api/v1/device-sessions`, `POST /api/v1/operator-sessions`, `POST /api/v1/operator-sessions/end` and `POST /api/v1/device-enrollments`. **The first three satisfy (b1)**: their entire externally visible effect is the cookie, they hand back no secret, and they owe no constraint — which is why (b) had to become disjuncts rather than the conjunction v1.4.1 drafted. **That conjunction was wrong and briefly expelled three of its own members**: requiring a named UNIQUE *and* a once-received credential of every member would have thrown out the three session routes the class was written for, since nothing in `app_session` makes a repeated sign-in a failed INSERT — and nothing should, because a second device session is one more row on a chain the phone stops using (§5.1). **Only `POST /api/v1/device-enrollments` sits under (b2)**, because its effect outlives the response — a `device`, a `device_credential` and a use row — and it names **`device_enrollment_code_use (code_id)`**. A future member satisfies one disjunct or is not a member; a (b2) member names its own constraint or is not one. **Naming is required because "a UNIQUE constraint" alone is not a test** — every table here has a surrogate primary key, which is a UNIQUE, so a clause satisfied by `id uuid PRIMARY KEY` is satisfied by every route in the system. What (b2) needs is a constraint on the ACT: the columns that make a second execution of the same request a failed INSERT rather than a second effect. `device_enrollment_code_use (code_id)` is that; `device (id)` is not.

*(v1.4.0 widened (b) from "whose entire externally visible effect is a cookie" and added the UNIQUE conjunct, from E03-D07. The two routes that bead adds fall on opposite sides of the line and settle what the class actually tests for. **`POST …/invitations/redemptions` is OUT**: it writes a `membership` and an `operator_pin`, both of which outlive the response and neither of which is a cookie, so clause (a) refuses it — and v1.3.0's own forward-looking sentence about "E03-D07's enrollment redemption" does not reach it. It takes the row, with `replayIfSettled` running before the code is looked at so a retry replays rather than finding the code its own first call spent. **`POST …/device-enrollments` is IN**: its effect is a cookie plus three durable rows, which fails (b) as originally written, and the reason to admit it is §5.1's own — a stored replay would return a byte-identical `201` **with no cookie**, which is the phone-that-holds-nothing failure this class exists to prevent. **The UNIQUE conjunct is what keeps the widening narrow**: `UNIQUE (code_id)` on `device_enrollment_code_use` is a stronger exactly-once guarantee than the key it replaces, taken by the database, on the act itself — and a route that writes a book, a batch or a catalog row has no such constraint, so none enters the class through this door. The cost is a retry after a lost response finding the code spent, which is one more code issued on a flow a shop performs once per phone.)* **I6 asserts the header on all four; nothing asserts the absence of the row, and that is deliberate** — the row's absence is not a property a test can distinguish from a bug, so the class is stated here instead, where the rule is read.

**SIX MEMBERS AT v1.5.0 (E03-D11).** `POST /api/v1/privileged-sessions` and
`POST /api/v1/privileged-sessions/end` join under **(b1)**, which is the disjunct v1.4.2 had to
create: their entire externally visible effect is the cookie, they hand back no shown-once secret, and
they owe no named UNIQUE — and nothing in `app_session` makes a repeated sign-in a failed INSERT, nor
should it. This is v1.3.0's own prediction arriving one bead later than it named: *"the next one will
be E03-D06's password sign-in"*, which E03-D06 could not ship because the first factor had no owner.

**AND ONE STATEMENT ABOUT WHAT §5.3 STORES, WHICH IS NOT AN EXEMPTION** (v1.5.0). E03-D11's two
issuance routes — `POST /api/v1/invitations` and `POST /api/v1/device-enrollment-codes` — are in
**neither class**: each writes a row that outlives the response and is not a cookie, so clause (a)
refuses them exactly as it refuses `POST …/invitations/redemptions`, and each takes the header AND the
`request_idempotency` row. What is settled here is narrower and was previously unstated:

> **A SHOWN-ONCE SECRET IS PART OF THE RESPONSE AND NOT PART OF THE STORED RESPONSE, exactly as a
> `Set-Cookie` is.** `invitation` holds `sha256(code)` and nothing else (048 §7.1), so putting the
> plaintext in `request_idempotency.response_body` would place a live credential in a jsonb column on
> the one route whose whole custody story is *"shown once, stored nowhere"*. The idempotent closure
> returns the body without it and the code is spliced onto the response afterwards, so **a replay
> returns the stored body truthfully — an id, an expiry and no code** — and the owner issues another
> and lets the first expire, which is what the CLI this route replaces already tells them to do.

**The cost is the same shape §5.1 already accepts for class one**, and it is stated rather than
discovered: a retry after a lost response cannot recover the code. The alternative was a credential in
a table, and the third option — pretending the replay is byte-identical — would be a lie in a
generated artifact (§3.3 property 3). **The response DTO declares `code` optional and says why**, so
the difference is in the contract rather than in a comment.

**A SECOND EXEMPTION CLASS, AND IT IS A DIFFERENT KIND OF OBJECT FROM THE FIRST (v1.4.3, from E03-B06).**

> **CLASS TWO — AN INBOUND PROVIDER CALLBACK. A request whose caller is a THIRD-PARTY SYSTEM that cannot be made to send a Longbox header, whose authenticity is a SIGNATURE OVER ITS OWN BYTES verified before any row is read or written, and whose exactly-once guarantee is a UNIQUE constraint on THE ACT that the member NAMES, requires no `Idempotency-Key` and takes no `request_idempotency` row.**

**Why this is a class and not a member of the first one.** Class one is about a route whose EFFECT the storage cannot hold — an authentication act's effect is a `Set-Cookie`, and replaying a stored `201` would hand a phone a success with no cookie. Class two is about a route whose CALLER cannot participate: Shopify's servers send no cookie, no `Sec-Fetch-Site` and no `Idempotency-Key`, and no amount of configuration makes them. **Collapsing the two would lose the distinction that matters** — class one's members COULD send the header and do; class two's cannot, at all, ever.

**The header requirement is not dropped, it is REPLACED, and by strictly stronger things.** §5.1's own reason for keeping the header on the credential routes is 048 §5.2's second CSRF mechanism: *"a cross-site HTML form cannot set a custom header."* For class two the equivalent statement is *a cross-site HTML form cannot produce a valid HMAC over the body under a secret it does not have* — which is stronger, not weaker. And where class one's (b2) accepts a named UNIQUE as a stronger exactly-once guarantee than a caller-chosen key, class two REQUIRES one, for the same reason and with the same naming rule: **"a UNIQUE constraint" alone is not a test**, because every table here has a surrogate primary key.

**THE TWO MEMBERS, each with its constraint** (E03-B06, 053 §8.3):

| Route | Its exactly-once constraint |
| --- | --- |
| `GET /api/v1/connectors/shopify/callback` | `connector_install_state_use (state_id)`. **The constraint guarantees EXACTLY-ONCE and deliberately not SPEND-ON-PRESENTATION**: the use row is written in the transaction that produces the effect, so a call failing before that point spends nothing and the same value stays re-attemptable (053 §5.2). A member must not "tidy" that into an earlier write — it turns a transient failure into a dead act and prevents nothing the UNIQUE does not already prevent |
| `POST /api/v1/connectors/shopify/webhooks` | `connector_webhook_receipt (connector, webhook_id)` |

**The boundary, so it cannot spread.** A member (a) has a caller that is a third-party system rather than a Longbox client or a browser holding a Longbox session — asserted as `principal: "none"` AND `kind: "provider-callback"` on the AUTH allowlist, and both are checked; **and** (b) verifies a signature over its own bytes before any database read or write; **and** (c) names a UNIQUE index on the act, which a contract test reads out of `migrations/` rather than taking on trust. **A route that has a session to check is not a member**, however awkward its client finds the header — the class is about who is calling, not about convenience. **Unlike class one, this class asserts its own membership**: `tests/contract/api-contract.test.ts` fails when a member names an index that does not exist, and `tests/contract/csrf-mechanisms.test.ts` fails when a `provider-callback` row carries a principal.
...
request_idempotency(
  ...
  response_status  integer,              -- NULL until completion (v1.2.0); NULL means in flight
  response_body    jsonb,                -- NULL until completion (v1.2.0); NULL means in flight
...
**The two response columns are NULLABLE, corrected at v1.2.0.** v1.1.x wrote them `NOT NULL`, which contradicts §5.3 step 1 one section below: the row is INSERTed before the work is done, so at INSERT time there is no response to hold. **A NULL response means in flight**, and that is not the status column §5.3 removed — §5.3's whole argument is that nothing outside the writing transaction can ever observe such a row, because a same-key request blocks on the unique constraint until the first transaction commits or rolls back. Found by execution: `migrations/009` could not be written as specified.
...
**SIX POSITIONS AT v1.6.0 (E03-D22), extending the two this section drafted, the three 048 K1 made,
the four E03-D06 left and the five E03-D11 left:**

> **`request_idempotency` INSERT → `app_session` (`FOR NO KEY UPDATE`) → `user_credential`
> (`FOR UPDATE`) → `user_authenticator` (`FOR UPDATE`) → `shop` (the connector store claim,
> `UPDATE`) → `scan_session` anchor (`FOR UPDATE`).**

The request's IDENTITY, the session that says who is asking, the first factor and then the second
saying they are still who they claim, then the TENANT ROW the act configures, and only then the
domain subject.

**The FIFTH position is E03-D22's and it is a WRITE rather than a `FOR UPDATE`, which is the point.**
`claimStoreDomain` writes `shop.shopify_domain` inside the connector install callback's transaction;
that takes a row lock on the shop's own row AND — the half that carries the guarantee — waits on
`shop_shopify_domain_is_one_store` while a concurrent claimant is uncommitted, which is how two
installs of one store become one winner and one `23505` (056 §11 R10, 000-docs/061 §3). **It sits
AFTER the three identity positions** because the authentication hook takes `app_session` before any
handler body exists and the privileged sign-in takes both credential locks while establishing the
session a shop-config write would run under — so nothing a handler takes can precede them, and this
is the lint pinning the order the code already has. **It sits BEFORE the `scan_session` anchor** on
coarse-before-fine, and the hierarchy is real: a `scan_session` belongs to a shop, so a handler
holding the finer lock while reaching for the tenant row would deadlock against the install callback,
which takes the tenant row first. **No chunk in the tree takes it together with any other position
today** — a provider callback carries no `Idempotency-Key`, no cookie and no scan session — so it is
registered ahead of its first pairing for the reason the authenticator position was, and it carries
one positive and two negative fixtures because a rule that has never failed on its own new pair is
indistinguishable from one that cannot. **The two credential positions
are not notional and are not independent:** the privileged sign-in takes BOTH in one transaction,
because 048 §4.3's per-person lockout budget is shared across all three factors and a shared budget
over two anchors is the write-skew 048 §9.1 exists to close. First factor before second is the order
the FLOW has anyway — a code presented without a password must not consume a step (048 R19) — so the
lint pins the order the code already wants rather than imposing one on it. I22's lint checks all six
over every PAIR, so a handler taking only two of them is still policed.
...
3. ~~**Should `v1` be RFC 9457 `problem+json`?**~~ — **ANSWERED: no, as drafted, and the margin stays thin.**
   Neither lens moved it. `type` as a dereferenceable URI is a hosting commitment this project has not made,
