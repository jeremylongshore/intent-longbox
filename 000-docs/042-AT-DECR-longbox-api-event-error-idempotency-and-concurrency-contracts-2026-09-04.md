...
**FIVE POSITIONS AT v1.5.0 (E03-D11), extending the two this section drafted, the three 048 K1 made
and the four E03-D06 left:**

> **`request_idempotency` INSERT → `app_session` (`FOR NO KEY UPDATE`) → `user_credential`
> (`FOR UPDATE`) → `user_authenticator` (`FOR UPDATE`) → `scan_session` anchor (`FOR UPDATE`).**

The request's IDENTITY, the session that says who is asking, the first factor and then the second
saying they are still who they claim, and only then the domain subject. **The two credential positions
...
lint pins the order the code already wants rather than imposing one on it. I22's lint checks all five
