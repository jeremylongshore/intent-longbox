...
Fixtures live in `tests/fixtures/schema/`, one per released schema, generated by `pnpm fixture:schema --upto NNN --out …` (a `pg_dump` of a database migrated to that point plus a small fixed-uuid seed) and **checked in**. `tests/integration/prior-snapshot-upgrade.test.ts` restores each, runs `pnpm migrate` forward, and asserts:
