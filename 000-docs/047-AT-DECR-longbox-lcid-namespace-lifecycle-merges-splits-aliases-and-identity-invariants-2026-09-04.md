...
| **1.1.2** | **2026-09-04** | Patch (statements of fact, from E04-D01's build and its invariant review): §4.2's registry column is `vertical_code` — 047 §3.1's three-character segment and 030 §3.2's discriminator cannot be one column, so `vertical_pack` carries both and the registry FKs the code; §10.2: an earned split continuation resolves as `merged` today and should carry its `split_id` (E04-D01 fix-up); §11's ten test files consolidated into five (`lcid-registry`, `lcid-lifecycle`, `crosswalk-catalog-authority`, `lcid-format`, `edition-signature`) — the RTM carries the mapping; A2's benchmark run (I18) is owned by E04-B04, the fixture is shape-only until a run on named hardware; corpus as-of comparisons are `(built_at, id)` tuples because `built_at` is transaction time and can tie. | acting head |
...
lcid_registry(
  ...
  vertical_code               text NOT NULL REFERENCES vertical_pack(vertical_code),   -- v1.1.2: the three-character LCID segment; vertical_pack also carries 030's `vertical` discriminator
