...
**Scope of this commit:** **the RECORD HALF ONLY.** No `src/`, `migrations/`, `scripts/` or `tests/*.ts` change lands with it. The identity substrate this record's actor model rests on — `app_user`, `membership`, `device`, `app_session` and its rotation chains, `operator_pin` — is landing under **E03-B02** (doc 048, RATIFIED v1.1.0) in a separate PR that owns migrations `019+`. **This record cites that substrate; it does not create it, and the code half of E03-B05 is a follow-on commit on this same bead.**
...
> **RULING (acting head). The environment variable per `key_ref` STAYS as the process-visible form. Its ONLY legitimate source is a SOPS-encrypted file decrypted at DEPLOY time into a root-owned, mode-600 `EnvironmentFile` that the unit declares mandatorily. The process never decrypts, never reads a SOPS file, and never writes one. The encrypted database column is REJECTED outright and this record closes the question.**
...
**The corollary that makes this a decision rather than a description of today.** A variable is legitimate only if a SOPS file is its source. A key typed into a shell, exported by hand, or added to a unit file directly is a key with no custody record — and there is no way for the process to tell the difference. So the check is at the *deploy* boundary, not the process boundary: the deploy step fails when the unit's `EnvironmentFile` contains a `LONGBOX_*` name the SOPS file does not declare. That is E13's to build; it is named here so it is not discovered as a gap (§13).
...
> |---|---|---|
> | **Environment variable** (the ruling in Q1) | (a) the `LONGBOX_*` line removed from the SOPS file and the file re-encrypted; (b) redeploy so the `EnvironmentFile` no longer carries it; (c) **process restart**; (d) the retirement row from Q2 | The value remains in every prior SOPS revision in git and in every backup of the repository. Nothing here reaches the provider |
> | **SOPS file decrypted in memory** (rejected in Q1, stated for completeness) | the same (a)–(d), minus the restart only if the process re-reads per call | identical residual |
...
> **A key value exists in exactly one place inside this system: a process environment variable whose name is a legal `key_ref` for exactly one shop. Its only legitimate source is a SOPS-encrypted file decrypted at deploy time into a root-owned mode-600 `EnvironmentFile` the unit declares mandatorily. The process never decrypts a SOPS file, never writes one, and never persists a key value to disk, to a column, to a log or to a response.**
...
The four steps and their receipt are ruled in §2 Q3. Two additions the code half owns:
...
| **G-1/G-2/G-3** — authentication, RBAC, RLS | **E03-B02/B03/B04** | The actor model this record's `authored_by` depends on. **This record cites doc 048's substrate and creates none of it** |
...
| **I10** | **Offboarding produces a receipt that names what was made unreachable and does not claim what it cannot know.** A retirement row with `reason_code='offboarding'` exists; the receipt text names the four steps; and **no field or sentence asserts that the provider-side credential was revoked** | `tests/integration/offboarding-receipt.test.ts` |
...
**Names only. Numbers are assigned when the file is written, never reserved in advance** — 044's ledger keys on the filename and E03-B02's PR owns `019+`. Every one is an **expand** step under 044 §2; none carries a `-- contract:` header.
...
7. **The deploy-boundary check** that a `LONGBOX_*` variable in a unit's `EnvironmentFile` is declared in the SOPS file — **E13**, named in §2 Q1 so it is not discovered as a gap.
