...
**Version:** 1.0.4
**Status:** PROPOSED — **`security-auditor` and `rich-hickey-reviewer` were DISPATCHED on the §3 ruling and both returned ACCEPT-WITH-CHANGES; the acting head's ruling STANDS, and v1.0.1 folded every finding in.** **§2's deliberation text is the session's own composition; the lens VERDICTS and DISSENTS quoted in §14.1 are dispatched and are the reopening triggers.** Both dissents are preserved verbatim in §14.1. Not citable as settled until §16 is signed after `longbox-gate-auditor` (record half) and `longbox-invariant-reviewer` (code half).
