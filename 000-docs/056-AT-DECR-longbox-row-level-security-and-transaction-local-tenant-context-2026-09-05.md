...
**FOUR of the NINETEEN** service-scoped tables are WRITTEN inside a scope; the other fifteen are only read. *(Kept verbatim at v1.1.0 as “Three of the eighteen … the other fifteen are only read”, and amended at v1.3.0: `app_user` joined both counts — `src/db/rowLevelSecurity.ts` is the declaration and says so.)* The write
