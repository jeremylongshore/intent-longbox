...
**Sensitivity:** Restricted internal when written, public since 2026-09-15 (014 §10). It names no shop, no person and no credential; it does name every table that carries a tenant, which is a map of the schema and not of anybody's data.
...
|---|---|---|---|
| **1.4.0** | **2026-09-06** | **MINOR — E03-B08 `longbox-e5b.3.8` (000-docs/064 §7.2) puts a `service_write` policy on `outbox`, which this record's §5.3 REJECTED IN SO MANY WORDS. Amend-by-row: every original sentence stays verbatim and three amendment blocks (§5.0, §5.3, I13) carry the change.** The gate audit's B1: at v1.3.0 §5.3 reads *"**`outbox` gets NO service policy**"* and rejects the alternative BECAUSE it "would put a permissive policy on `outbox`, the busiest shop-scoped table in the system" — and E03-B08 does exactly that. It is a MINOR and not a patch because a decision moved, and the amendment says which one and on what ground: **§5.3's rejection was about a cross-tenant CLAIM inside the poller's scope, and it stands** — the drain is still a per-shop loop under a tenant context, and nothing about it changed. What is new is a DIFFERENT question that record did not face: the connector's inbound transaction cannot be a tenant one (053 §5.5 — the tenant may be NULL), and 043 §2.1 requires the obligation and its job to commit together, so the enqueue is either inside that scope or the fact and its effect come apart. The WRITE is pinned to ONE event name by the policy predicate itself, so the inbound scope can enqueue the privacy job and can never enqueue a draft. The READ is the whole table and is NOT narrowable in this shape — a read policy is a scope list, not a row condition — and that is the widening a reviewer should weigh: an `outbox` row carries a reference and an envelope and never the referenced row's values (043 §2.5), and this scope already reads every shop's `connector_token_version` cross-tenant by design. **Three counts move**: §5.0's *four of the nineteen* → **six of the twenty-one**, and I13's write list gains `outbox` and `privacy_request`. | E03-B08 → `longbox-security-tenancy-builder` |
| **1.3.0** | **2026-09-06** | **MINOR — §11 R9 is DISCHARGED for `app_user`, and §5's closed union gains an EIGHTH member. Amend-by-row: every original sentence stays verbatim and each amendment is written beside it.** `migrations/036` (E03-D21 `longbox-e5b.3.31`) policies the person table on `EXISTS (SELECT 1 FROM membership m WHERE m.app_user_id = app_user.id AND m.shop_id = current_shop_id() AND <the liveness triple>)` — the parent-EXISTS shape §7 already names for `retention_hold_release`, with `membership_revocation` INSIDE the `NOT EXISTS` for §6.1's fail-open reason. **The ground R9 gave was half wrong and the half it rested on is the wrong half**: `grantInvitation` writes the membership BEFORE the person is read, in one transaction, so redemption needed no change; ISSUANCE is what has no grant to point at, and it is answered by the eighth scope, **`person-admission`** (one call site, one table, `SELECT` + an `INSERT` checked on `status = 'active'`, no UPDATE and no DELETE) rather than by a hole. **A MINOR and not a patch, on this record's own convention**: the closed union in §5 is a DECISION and its membership changed, and R5's own sentence — *"An eighth belongs in this record with its reason before it belongs in the union"* — asks for exactly the amendment now written beside it. **Five tables stay exempt and their reason is NEW rather than inherited** (`user_credential`, `user_authenticator`, its retirement, `recovery_code`, `recovery_code_use`): a delegated reason pointing at a row that has moved is a reason nobody has checked, so each is re-argued in 000-docs/062 §4 — and `user_authenticator`'s ground is that a policy there would be HARMFUL, because 048 R19 makes an affected-row count the authorization. **No ruling in this record is reversed, no invariant is re-scoped, no threshold moves**, and §13's alternatives stand verbatim. **TWO defects in this record's own boot assertion were found on the way, and the second one is the boundary (062 §5).** The first was a FALSE POSITIVE: the SQL and TypeScript halves of the policy-predicate normaliser disagreed about non-space WHITESPACE, so a predicate long enough for Postgres to WRAP was reported ALTERED and refused a port — fail-closed, and still wrong. The second was found by the cannon and is a FALSE NEGATIVE: **both** halves stripped PARENTHESES, so a policy relaxed only by removing the grouping around this record's own `effective_until` disjunction compared EQUAL to the declared text, booted green, and returned every person in the estate to any shop holding one time-bounded grant. **§4's SET comparison is therefore restated in mechanism**: the SQL normalisation is DELETED, `pg_policies` is read RAW, and ONE TypeScript function compares both sides as DEPTH-ANNOTATED TOKEN SEQUENCES — a comparison that erases operator precedence is not a boundary check. Two negative fixtures pin it (the relaxation must fail; the wrapped predicate must pass), and a third turns §7's *a policy here would be HARMFUL* into enforcement by refusing a boot on any policy over a DECLARED EXEMPTION. **No ruling of §4 is reversed** — the check is still a set comparison of what EXISTS against what this design EMITS; only the way two strings are compared changed, and it changed because the old way could not see the failure the check exists for. | E03-D21 → acting head |
| **1.2.3** | **2026-09-06** | **A PATCH, and a ROW plus TWO AMENDMENT BLOCKS — no original sentence is edited.** **§11 R10 is DISCHARGED** by E03-D22 `longbox-e5b.3.32`: the install now CLAIMS the store on `shop.shopify_domain` inside the transaction that introduces the token version, so two concurrent callbacks for one store become one winner and one `23505` decided by `shop_shopify_domain_is_one_store`, and the F8 read becomes a fast path rather than the guarantee. Both fixes R10 examined and rejected stay rejected and neither was reopened: the check is still not moved inside the transaction (it would fail OPEN under the tenant context, §6.1) and `service_write` is still not widened to `connector_token_version` (the security lens's F1). **§6.3 is NOT discharged, and this row says so rather than letting a reader infer it.** Its stated reason for being latent — *"no route writes the column"* — is now FALSE: there is exactly one writer, and the oracle is REACHABLE. What replaces the latency is a bound argued in 000-docs/061 §6 and offered as a SECOND general discharge route for §6.3's own rule: *a UNIQUE over a client-choosable value must either be prefixed by the tenant, OR the value must stop being freely choosable at that scope — by requiring the caller to prove control of it.* This index cannot take the first route, because its whole purpose is CROSS-TENANT uniqueness and a `shop_id` prefix would make it assert nothing; it takes the second, because reaching the claim costs a completed OAuth grant at the store being asked about. So the one bit learnable is *"is MY store already a Longbox customer"*, about a store the caller administers — not a probe over stores at large. **No migration was needed and none was written** (061 §7). Evidence: `tests/integration/connector-oauth.test.ts`, **35** cases with an **eight**-case E03-D22 block; re-run at `9900d7b` with the claim removed it reports **9 failed / 26 passed**, the eighth case correctly surviving because it measures the lock's cost rather than the claim's existence. | E03-D22 `longbox-e5b.3.32` → `longbox-security-tenancy-builder` |
...
> **AMENDED at v1.3.0 (E03-D21 `longbox-e5b.3.31`): THE SET COMPARISON STANDS AND ITS MECHANISM IS RESTATED, because the way two predicates were compared could not see the failure this check exists for.** The comparison normalised both sides **twice, in two languages** — `normalisePredicate()` in TypeScript and four `replace()` calls inside the catalog query — and **both deleted PARENTHESES**. The cannon reproduced what that costs on E03-D21's own policy: remove only the grouping around an `effective_until` disjunction and the relaxed text normalises to the declared text CHARACTER FOR CHARACTER, boots green, and returns **every person in the estate** to any shop holding one time-bounded grant. *A comparison that erases operator precedence is not a boundary check.* The SQL normalisation is DELETED, `pg_policies` is read RAW, and ONE TypeScript function compares both sides as **depth-annotated token sequences**; the cost — every declared predicate is now written in the deparser's shape, by hand — is stated in 000-docs/062 §7 R6, and the round-trip that removes it is **E03-D32**. **A SEVENTH condition joins the six above**: a table this design declares EXEMPT that nevertheless carries `relrowsecurity` or a policy of any kind, reported as its own array because its remedy is the opposite of every other one here — DROP the policy, do not add one. That is what turns §7's *a policy over `user_authenticator` would be HARMFUL* from an argument into a control. ⚠ **And the list of six was already short by one before this bead**: `forbiddenGrants` — a privilege held on a table declared `appGrant: "none"` (058 F2) or `"insert-only"` (060 F2), and now `"read-append"` (062 F1) — has been part of the check since E03-D14 and was never added here. Noted as a pre-existing omission rather than repaired silently. No ruling of this section is reversed: the check is still a SET comparison of what EXISTS against what this design EMITS. 000-docs/062 §5.
...
> **AMENDED at v1.3.0 (E03-D21 `longbox-e5b.3.31`):** **there is an EIGHTH, `person-admission`**, and the list below is kept verbatim as the record of the seven. It cannot name a shop for the reason §7's `app_user` bullet gives as its ground for having no policy at all: an invitation NAMES its addressee before any code is minted (048 §7.1), and that addressee may hold no grant anywhere — or may already work at ANOTHER shop, whose row the inviting shop must find by email and may not read. It is the narrowest member of the union: one call site, one table, `SELECT` plus an `INSERT` checked on `status = 'active'`, no UPDATE and no DELETE. §11 R5's amendment and **000-docs/062 §3.2** carry the full argument, including the two shapes refused.
...
**FOUR of the NINETEEN** service-scoped tables are WRITTEN inside a scope; the other fifteen are only read. *(AMENDED at v1.4.0, E03-B08: **SIX of the TWENTY-ONE**. `outbox` and `privacy_request` joined both counts — see the §5.3 block below and 064 §7.2. The declaration is still `src/db/rowLevelSecurity.ts` and still says so; `tests/rls-plan.test.ts` and `tests/integration/rls-tenant-isolation.test.ts` assert the six from the plan and from the LIVE catalog respectively.)* *(Kept verbatim at v1.1.0 as “Three of the eighteen … the other fifteen are only read”, and amended at v1.3.0: `app_user` joined both counts — `src/db/rowLevelSecurity.ts` is the declaration and says so.)* The write
...
> **AMENDMENT (v1.4.0, 2026-09-06, E03-B08 `longbox-e5b.3.8`, 000-docs/064 §7.2).
> `outbox` NOW CARRIES A `service_write` POLICY AND A `service_context` READ, AND
> THIS SECTION SAID IT WOULD NOT. Nothing above is edited; what follows is the
> narrower question that record did not face, and its answer.**
>
> **The rejection above STANDS and is not reopened.** It is about a cross-tenant
> CLAIM inside the poller's scope — one query per tick over every shop's rows —
> and the drain is still a per-shop loop under a tenant context. Nothing about
> `drainAllShops`, `claimBatch` or the attempt log changed.
>
> **The question E03-B08 faced is a different one.** The connector's inbound
> transaction CANNOT be a tenant one: 053 §5.5 rules that a webhook matching no
> install writes a NULL tenant, and one transaction carries one context. 043 §2.1
> requires the obligation (`privacy_request`) and the job that answers it to
> commit together. So the enqueue is either inside `connector-inbound`, or the
> fact and its effect come apart — which is the half-written chain the outbox
> exists to remove. The two alternatives were weighed and are recorded in 064
> §7.2: branching the context on whether the store resolved would put the receipt
> path's two most different cases on two code paths, and enqueueing after the
> commit splits the fact from its effect.
>
> **The WRITE is pinned to ONE EVENT NAME by the policy predicate**, which is the
> narrowest form this shape allows and is narrower than §5.0's other five:
> `((shop_id IS NOT NULL) AND (event = 'longbox.platform.privacy_request_received'))`.
> The inbound scope can enqueue that job and can never enqueue a draft. Two tests
> pin it — one over the plan, one over the LIVE catalog.
>
> **The READ is the whole table and is NOT narrowable here, and that is the
> widening a reviewer should weigh rather than a detail.** A `service_context`
> policy is a scope LIST and not a row condition, and `enqueue` needs the read
> because an `INSERT … RETURNING` is filtered by the SELECT policy. Two facts
> bound what it is worth: an `outbox` row carries a reference and an envelope and
> **never the referenced row's values** (043 §2.5), and this scope already reads
> every shop's `connector_token_version` cross-tenant by design — a strictly more
> sensitive table. It is a widening of DEGREE and not of kind, it is carried as
> 064 §9 R3, and the fix — a row condition on a service READ policy — is
> **E03-D35 `longbox-e5b.3.45`**, created on `main` at `8068a58`.
>
> **`privacy_request` is the sixth**, and it needs no argument beyond 053 §5.5's:
> its tenant may be NULL, so the write has nowhere else to happen. Its check
> requires `webhook_receipt_id IS NOT NULL`, so the scope can only record an
> obligation that cites the signed message which caused it.
...
> **AMENDMENT (v1.2.3, 2026-09-06, E03-D22 `longbox-e5b.3.32`, 000-docs/061 §6).
> THE DAY CAME. This section is NOT discharged, its latency reason is now FALSE,
> and a different bound replaces it. Everything above stays verbatim as the record
> of what was true until the claim landed.**
>
> `completeInstall` now writes `shop.shopify_domain` inside the transaction that
> introduces the token version, so **there is exactly one writer and the oracle is
> REACHABLE**. The amendment above guessed that the same change would "remove the
> oracle"; it does not, and the honest correction is worth more than the guess.
>
> **What this section asked for it cannot have.** Its rule offers ONE discharge
> route — prefix the UNIQUE with the tenant — and this index cannot take it:
> **its entire purpose is CROSS-TENANT uniqueness** (one store belongs to at most
> one Longbox shop, 053 §7.3), so a `shop_id` prefix would make it assert nothing.
> A rule with one route and a case that cannot take it is a rule that needs a
> second route, and 061 §6 argues one:
>
> > **A UNIQUE over a client-choosable value must either be PREFIXED BY THE
> > TENANT, or the value must STOP BEING FREELY CHOOSABLE at that scope — by
> > requiring the caller to prove control of it, where the proof is PER-PROBE and
> > NON-REPLAYABLE and the value is one the caller ADMINISTERS.**
>
> **The two qualifiers are part of the rule and not commentary on it.** Without
> *per-probe and non-replayable*, one proof unlocks many questions and the route
> becomes a scanner at the cost of a single grant. Without *administers*, the bit
> learned is about a third party rather than about the asker — which is exactly
> what this section objected to, so a version of the rule missing that word would
> discharge nothing.
>
> Here the proof is a completed OAuth grant: reaching the claim costs merchant
> consent at the store being asked about, minted through an install state an owner
> asked this system for, with the state's domain matched against the callback's
> twice (053 §8.1). Every probe needs a FRESH `connector_install_state` row and a
> REAL grant at the store being probed, and a spent state cannot be re-presented
> (053 §5.2). **So the one bit learnable is *"is MY store already a Longbox
> customer"* — about a store the caller administers — rather than commercial
> information about third parties.** The route cannot be turned into a scanner.
>
> ⚠ **THE RULE IS PROSE-ENFORCED. THERE IS NO LINT.** Nothing in `pnpm arch` or
> `pnpm migrate` fails when a `UNIQUE` is added over a client-choosable column on
> a policied table, and nothing checks that a new writer of such a column proves
> control first. This section is a rule a reviewer applies, not one the build
> applies, and the honest form of that is to say so beside it.
>
> **What is NOT claimed:** the index is still not tenant-prefixed, and a `23505`
> is still distinguishable from an `UPDATE 1` to anyone who can execute both.
> Nothing here makes that untrue; what changed is who can execute them, and about
> what. It is carried as 061 §9 R1.

---

## 7. Tables with no tenant are DECLARED, with a reason each

`src/db/rowLevelSecurity.ts` carries a row per shop-less table, and the live schema is asserted against that list **in both directions**. The classes, rather than the **24** rows *(22 when this record was written; `identity_access`'s neighbours and E03-D14's origin tables joined the list, and `app_user` left it at v1.3.0)*:
...
| **I2** | Every table carrying no `shop_id` is a declared exemption with a reason; a live table in neither class stops the migrate run. **AMENDED at v1.3.0 (E03-D21): a shop-less table now has THREE answers rather than two** — a tenant column of another name (`shop`), a PREDICATE that joins (`app_user`, `TENANT_PREDICATES`), or a declared exemption; `POLICIED_WITHOUT_SHOP_ID` is the one export all three readers use. The loud failure is unchanged, and the exemptions are now checked against the DATABASE as well as against the list: a policy or `relrowsecurity` on a declared exemption refuses the boot (062 §5.3). | same file; `rls-plan.test.ts` |
...
| **I13** | A service scope may READ only the tables that NAME it, and may WRITE only `auth_attempt`, `connector_token_retirement` and `connector_webhook_receipt`, each under a check that constrains the ROW and not merely the scope. **AMENDED at v1.3.0 (E03-D21): FOUR tables — `app_user` joined them, writable under `person-admission` alone and only with `status = 'active'`, which is what makes *the running server can never suspend or deactivate a person* a property of the database. The lane case asserts four.** **AMENDED at v1.4.0 (E03-B08): SIX tables — `outbox` and `privacy_request` joined them (§5.3's amendment block). `outbox`'s check is the narrowest of the six because it names an EVENT (`event = 'longbox.platform.privacy_request_received'`), so the inbound scope can enqueue the privacy job and can never enqueue a draft; `privacy_request`'s requires `webhook_receipt_id IS NOT NULL`, so an obligation must cite the signed message that caused it.** | same file, three cases: the policy SHAPE from the catalog; the F1 regression (a cross-tenant `membership` INSERT from inside `my-shops` is refused); and the write policies' BEHAVIOUR — a NULL-`shop_id` `auth_attempt` accepted under `session-resolution` and a shop-named one refused, the reverse under `code-redemption`, and a `connector_webhook_receipt` accepted under `connector-inbound` with and without a shop and refused under any other scope |
...
> **AMENDMENT (v1.3.0, E03-D21 `longbox-e5b.3.31`, 2026-09-06) — THE EIGHTH IS HERE, AND THIS IS THE SENTENCE R5 ASKED FOR.** The scope is **`person-admission`**, and its reason is the one §7's `app_user` bullet gives as the ground for having no policy at all. An invitation NAMES its addressee before any code is minted (048 §7.1), so `upsertPerson` runs for a person who may hold no grant anywhere — and, in the ordinary case, for a person who already works at ANOTHER shop, whose row this shop must find by email and may not read (034 §2.6). It is therefore the one moment `app_user`'s new membership-EXISTS policy cannot serve, and it is answered by a declared scope rather than by a hole. **It is the narrowest member of the union**: ONE call site (`api.ts`'s invitation route), held exact by `pnpm arch`; ONE table; `SELECT` plus an `INSERT` whose check is `status = 'active'`, which — with no UPDATE or DELETE reachable inside it — makes *the running server can admit a person and can never rename, suspend or deactivate one* a property of the database. The CLIs enter no scope, because the schema owner bypasses (§4). Full argument, including the two shapes refused (a `WITH CHECK (true)` INSERT, and a context that CHANGES inside the request transaction): **000-docs/062 §3.2**. The count in this residual's first sentence is now **eight**; every other sentence of R5 stands verbatim.
...
> **AMENDMENT (v1.2.3, 2026-09-06, E03-D22 `longbox-e5b.3.32`, 000-docs/061).
> R10 IS DISCHARGED. Everything above is kept verbatim as the record of what was
> true until the claim landed.**
>
> `completeInstall` now writes `shop.shopify_domain` for the shop the install
> state named, as the FIRST statement of the transaction that introduces the
> token version, under that shop's own tenant context. Two concurrent callbacks
> for one store are therefore decided by `shop_shopify_domain_is_one_store`: one
> commits, the other raises `23505`, and its transaction rolls back whole — no
> claim, no token version, no `connector_install_state_use` row, and a state that
> is still unspent (053 §5.2's H4), so the merchant can be sent back through.
>
> **Both fixes this residual examined stay REJECTED and neither was reopened.**
> The F8 read is still outside the transaction — moving it in would still fail
> OPEN under the tenant context, §6.1 — and `service_write` is still NOT widened
> to `connector_token_version`, so F1's line holds. What changed is that the
> serialisation point this residual named in its own last sentence is now taken.
>
> **Three things about the shape, because a discharge that hides its edges is a
> weaker record than one that shows them.** (i) **The row lock is not the
> mechanism**: two shops are two `shop` rows and their row locks never conflict;
> what serialises them is the btree unique waiting on an uncommitted index entry
> — so "optimising" this into a `SELECT … FOR UPDATE` on the shop row would
> remove the guarantee and keep its appearance. (ii) **The row count is the
> authorization** (§6.2): the `UPDATE` is followed by a check that exactly one row
> was affected, because a statement that matches nothing succeeds. (iii) **The F8
> read SURVIVES as a fast path** and is no longer the guarantee — it refuses
> before the authorization code is exchanged, which is worth a round trip, and
> both raise sites map to ONE refusal member and one wire code, so a caller cannot
> tell a store claimed a second ago from one claimed a month ago.
>
> **What is NOT discharged is §6.3**, above: its latency reason is now false and a
> different bound replaces it. And **the claim is NOT released by an uninstall** —
> 061 §4 rules on that, on 034 §4.2's ground that the column is configuration and
> a third party's act must not be able to un-configure a shop; the cost, that
> moving a store to a different Longbox shop needs a schema-owner act, is 061 §9
> R2 and belongs to the beads that own winding a shop down — `longbox-e5b.16.1`
> with E03-B09 `longbox-e5b.3.9` — beside R8, **which names E15 for the same act
> and is wrong in the same way; see the note below**.
>
> Evidence: `tests/integration/connector-oauth.test.ts` (35 cases, eight new and
> three amended), including a deterministic race that holds the first claim open
> on its own connection; re-run at `9900d7b` with the claim removed the same file
> reports **9 failed / 26 passed**, one E03-D22 case correctly surviving because
> it measures the lock's cost rather than the claim's existence.
>
> **ONE ERROR IN THIS RECORD IS NOTED AND DELIBERATELY NOT FIXED HERE.** §11 **R8**
> says an undrained outbox *"becomes real the day offboarding lands (E15)"*, and
> `longbox-e5b.15` is **CI/CD and controlled release** — it owns none of that. The
> same mistake was made by 000-docs/061 v1.0.0 and corrected there (both reviewers
> caught it). It is recorded for **this record's next bump** rather than repaired
> by a bead amending a different section: R8 is not E03-D22's, and a builder
> editing a residual it does not own is how a ratified record drifts. The owner is
> `longbox-e5b.16.1` with **E03-B09** `longbox-e5b.3.9`.
...
> **AMENDMENT (v1.3.0, E03-D21 `longbox-e5b.3.31`, 2026-09-06) — R9 IS DISCHARGED FOR `app_user`, AND NARROWED TO A DIFFERENT CLASS FOR THE REST.** `migrations/036` policies `app_user` on exactly the predicate R9 called expressible: `EXISTS (SELECT 1 FROM membership m WHERE m.app_user_id = app_user.id AND m.shop_id = current_shop_id() AND <the liveness triple>)`, on both halves, with `membership_revocation` inside the `NOT EXISTS` for §6.1's reason. **The transaction R9 rested on turned out not to be the obstacle it described**: `grantInvitation` INSERTs the membership and `resolvePersonByKey` reads the person AFTERWARDS, in the same transaction, so the row the policy looks for is already there when it looks — the redemption path needed no change at all. What genuinely has no grant to point at is ISSUANCE, and that is the eighth scope R5's amendment names. **The five other tables R9 listed are NOT policied and their reason is no longer this one.** Each is re-decided on its own ground in 000-docs/062 §4, and two of those grounds are worth carrying here: every application-role path to a credential table is inside the `second-factor` scope, where a tenant predicate is vacuous by construction — so a policy there would be in force on no path the running server takes — and `user_authenticator` is stronger than that, because 048 R19 makes the AFFECTED-ROW COUNT of its `UPDATE … WHERE last_used_step < $2` the authorization, and a row filter over that statement turns a misconfiguration into a refusal indistinguishable from a replay. **For those five the honest statement is unchanged**: bounded by code, not by the database — now with five separate reasons rather than one delegated one. Carried as **000-docs/062 §7 R4**.
