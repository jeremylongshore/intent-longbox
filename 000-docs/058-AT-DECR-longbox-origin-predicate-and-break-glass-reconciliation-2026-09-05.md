...
**UNSIGNED.** The cannon is dispatched and folded (§2, §13, §14); the gate audit's fact repairs are applied; `longbox-invariant-reviewer` audits the code half and `longbox-gate-auditor` re-audits this record. **The acting head signs at CLOSE with the merge SHA**, and 000-docs/006 carries the row. Until then no artifact may cite this record as ratified, and every citation of T35(c) that leans on it must carry §7's *application sessions* qualification and §11 R3's *not yet scheduled*.
