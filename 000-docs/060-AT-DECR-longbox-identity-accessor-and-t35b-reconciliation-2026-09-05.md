...
**Sensitivity:** Restricted internal when written, public since 2026-09-15 (014 §10). It names tables, columns, functions and commands; it contains no person's name, no identifier of any live person, and no secret value of any kind.
...
| **R5** | **`app_user` still carries no tenant policy** (056 §11 R9). The module graph, this accessor and every read being by the authenticated person's own id or by a digest are what bound it. Unchanged by this bead, and narrowed by it: there is now one door instead of five statements. | **DISCHARGED 2026-09-06 by E03-D21 `longbox-e5b.3.31`** — `migrations/036` policies `app_user` on a live membership one join away (000-docs/062; 056 v1.3.0 §11 R9). **The accessor is unchanged and its four reads return exactly what they returned before**: each already ran under an ordinary tenant context, for a person who holds a live grant at that shop by construction. The policy now sits UNDER the door rather than beside it, so a statement that reached the person table without going through `src/identity/` no longer merely fails a text rule — it returns nothing. The residual that REPLACES this one is narrower and is 062 §7 R4: the five credential tables are still bounded by code. |
...
**RATIFIED at v1.1.2, 2026-09-06.** Signed by the acting head at close of E03-D17 with the merge SHA `43d8ba87c372fd7a4d0517d204fd5f8dde9ec854` (PR #97, 8/8 required checks; 1784 unit / 810+1 integration). The signature authenticates §3–§8 as decided — the identity accessor as the one door to a person, `identity_access` as a fact about the ACCESSOR and never the subject, the closed purpose enum, the insert-only privilege class now asserted at boot, the seven person-projection columns gated from any table, hour-truncated facts — §2 as the lenses' own words, §9's declined alternatives and §12's preserved positions, and §11 as the honest residue: 019 T35(b) is INSTRUMENTED and not closed; the break-glass gate half is E11-B09's, the periodic run is E13-B04-D1's, and audit completeness cannot be proven here, only asserted (R6). The paragraph below is the PROPOSED-era text, kept as history.

**PROPOSED — NOT ratified at v1.1.1 (as written). §13 was UNSIGNED.** The two-lens cannon was dispatched and both lenses returned ACCEPT-WITH-CHANGES; all eight findings are folded, none is declined, and §12 preserves the three positions adopted differently or not at all. A `longbox-gate-auditor` NOT-READY is folded in the same pass — nine dead section cites, two wrong counts, one wrong section number, the missing 058 row and the missing gate-config exemption, plus F7's enum widening, which was a decision and not a repair.
