  ...
  try {
    ...
    console.log(
      `grants  ${role}: ${plan.appendOnly.length} append-only (SELECT, INSERT), ` +
        `${plan.mutable.length} exempt (full DML), ${plan.columnScoped.length} exempt (SELECT, INSERT + UPDATE on named columns), ${plan.noGrant.length} exempt (no grant), ${plan.insertOnly.length} insert-only (INSERT, no SELECT), ${plan.readAppend.length} read-append (SELECT, INSERT; no UPDATE, no DELETE), ` +
